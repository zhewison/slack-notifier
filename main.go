...
var (
	botToken     = flag.String("bot_token", os.Getenv("SLACK_BOT_TOKEN"), "Slack bot token")
...
func main() {
	...
	if *botToken == "" {
		flag.PrintDefaults()
		log.Fatal("-bot_token or SLACK_BOT_TOKEN required")
	}
	if *userName == "" {
		flag.PrintDefaults()
	...
	notifier := NewSlackNotifier(*botToken)
	err := notifier.Notify(payload)
	if err != nil {
		fmt.Println("slack-notifier: Failed to sent message to slack. Error: ", err.Error())
	} else {
		fmt.Println("slack-notifier: Sent message to slack")
	}
}
